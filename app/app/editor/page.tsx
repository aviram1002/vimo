...
export default function EditorPage() {
  ...
  // Event form
  const [eventName, setEventName] = useState('החתונה שלנו')
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex items-center justify-center bg-slate-100 relative"
            ...
            <div className="w-64 rounded-[36px] overflow-hidden flex flex-col"
              ...
              <div className="flex-1 px-5 py-4 flex flex-col items-center justify-center gap-3 text-center min-h-[440px]">
                ...
                <div className="font-black text-brand-purple" style={{ fontSize:`${fontSize}px`, background:'linear-gradient(135deg,#6C3BFF,#3BD1C6)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>
                  {eventName}
                </div>
                ...
                {date && <div className="text-base font-black text-brand-purple">{date}</div>}
                {venue && <div className="text-sm text-brand-muted">{venue}</div>}
                <button className="w-full py-2.5 rounded-xl text-white text-sm font-bold mt-2"
          ...
          <div className="w-80 bg-white border-r border-brand-border flex flex-col">
            <div className="flex border-b border-brand-border">
              {(['text','colors','fonts'] as PanelTab[]).map(tab => (
                <button key={tab} onClick={() => setActivePanel(tab)}
                  className={clsx('flex-1 py-3 text-xs font-semibold transition-colors relative',
                    activePanel === tab ? 'text-brand-purple' : 'text-brand-muted hover:text-brand-text')}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {activePanel === 'text' && (
                <>
                  <Input label="שם האירוע" value={eventName} onChange={e => setEventName(e.target.value)} />
                  ...
                  <Input label="מיקום" value={venue} onChange={e => setVenue(e.target.value)} placeholder="שם האולם, עיר" />
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {['💍','🎂','🎉','✡️','🏢','🌸','🎊','❤️'].map(e => (
                        <button key={e} onClick={() => setEmoji(e)}
                          className={clsx('w-10 h-10 text-xl rounded-xl border-2 transition-all',
                            emoji === e ? 'border-brand-purple bg-violet-50' : 'border-brand-border hover:border-brand-purple')}>
                          {e}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="text-sm font-semibold text-brand-muted block mb-1">גודל גופן</label>
                    <div className="flex items-center gap-3">
                      <button onClick={() => setFontSize(Math.max(12, fontSize-2))} className="w-8 h-8 rounded-lg border border-brand-border bg-brand-bg hover:border-brand-purple transition-all flex items-center justify-center">−</button>
                      <span className="text-base font-bold w-8 text-center">{fontSize}</span>
                      <button onClick={() => setFontSize(Math.min(48, fontSize+2))} className="w-8 h-8 rounded-lg border border-brand-border bg-brand-bg hover:border-brand-purple transition-all flex items-center justify-center">+</button>
                    </div>
                  </div>
                </>
              )}
              {activePanel === 'colors' && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[
                      'linear-gradient(135deg,#6C3BFF,#3BD1C6)',
                      'linear-gradient(160deg,#fdf6ff,#f0eeff)',
                      'linear-gradient(160deg,#fff0f8,#ffe0f0)',
                      'linear-gradient(160deg,#0f0f1a,#1a1a2e)',
                      'linear-gradient(160deg,#e0f7fa,#b2ebf2)',
                      'linear-gradient(135deg,#FF4FA3,#FF8C42)',
                    ].map((p,i) => (
              ...
              {activePanel === 'fonts' && (
                <div className="space-y-3">
                  {['Heebo – עברי נקי','Assistant – קל ואוורירי','Frank Ruhl – קלאסי'].map(f => (
                    <button key={f} className="w-full p-3 rounded-xl border border-brand-border bg-brand-bg text-sm font-semibold hover:border-brand-purple hover:text-brand-purple hover:bg-violet-50 transition-all text-right">
                      {f}
                    </button>
                  ))}
                </div>
      ...
      <div className="p-6">
        <div className="flex gap-2 mb-6 flex-wrap">
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setFilter(cat.id)}
              className={clsx('px-5 py-2 rounded-full text-sm font-bold transition-all duration-150',
                activeFilter === cat.id ? 'text-white shadow-primary' : 'bg-white border border-brand-border text-brand-muted hover:border-brand-purple hover:text-brand-purple')}
        ...
        {templates.length === 0 ? (
          <div className="text-center py-20 text-brand-muted">
            ...
            <p className="font-bold">טוען תבניות...</p>
          </div>
        ) : (
            ...
            {filtered.map(t => (
              <div key={t.id} className="card overflow-hidden cursor-pointer group">
                <div className="relative h-72 overflow-hidden" style={{background:t.preview_css}}>
                  {t.is_premium && <div className="absolute top-3 right-3"><Badge variant="premium">Premium</Badge></div>}
